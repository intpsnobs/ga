...
def one_point(parent_1: np.ndarray, parent_2: np.ndarray, index: int = -1) -> np.ndarray:
    if index == -1:
        index = int(np.random.uniform(low=1, high=len(parent_1)-1))
    return np.array([np.hstack([parent_1[:index], parent_2[index:]]), np.hstack([parent_2[:index], parent_1[index:]])])
